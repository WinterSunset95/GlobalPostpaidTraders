<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
	<title>Subscriptions</title>
	<link rel="stylesheet" href="./css/global.css?version=1.1">
	<link rel="stylesheet" href="./css/nav.css?version=1.1">
	<link rel="stylesheet" href="./css/subscriptions.css?version=1.1">
</head>
<body>

	<?php include 'header.php' ?>
	
	<main>
		<h1>Subscriptions</h1>
		<ol class="non-nav-list">
			<li>Subscription to Postpaid Trade Analysts is daily basis. Next day tip will be provided only to clients who clears the days’ trading premium. So traders are advised to clear charges at the end of the trading day to avail of the next day trading tip on time.</li>
			<!--
			-->
			<li>For Subscription, clients can pay to the following payment gateways:     </li>
			<ol class="child_list">
				<li>GPay no. 7629093811 or Scan the following QR Code: <br>
					<img src="./assets/gpay_qr.jpeg" alt=""> <br>
					For every payment, screenshot copy should be sent immediately to this number.
				</li>
			</ol>
		</ol>
	</main>


</body>
</html>
